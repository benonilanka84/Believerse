...
export default function CreatePost({ user, onPostCreated, fellowshipId = null }) {
  ...
  const handlePost = async () => {
	...
	fellowship_id: fellowshipId
      ...
      setLoading(false); // Reset button even if error!
    }
  };
  ...
  return (
    <div style={{ background: "white", padding: "25px", borderRadius: "12px", marginBottom: "20px", boxShadow: "0 4px 15px rgba(0,0,0,0.1)" }}>
      ...
      <div style={{ marginBottom: "15px" }}>
        ...
        <div onClick={() => fileInputRef.current.click()} style={{ padding: "10px", border: "1px dashed #ccc", borderRadius: "8px", textAlign: "center", color: "#666", fontSize: "13px", cursor: "pointer", background: mediaFile ? "#e8f5e9" : "transparent" }}>
          {mediaFile ? `✅ ${mediaFile.name}` : "📷 Add Image/Video"}
        </div>
      </div>

      <div style={{ textAlign: "right" }}>
        <button onClick={handlePost} disabled={loading} style={{ background: "#2e8b57", color: "white", padding: "10px 25px", borderRadius: "8px", border: "none", fontWeight: "bold", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
