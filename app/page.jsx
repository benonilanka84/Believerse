  ...
  return (
    <div className="container grid md:grid-cols-2 gap-6 items-center">
      ...
      <div className="card">
        ...
        <input className="w-full my-2 p-2 rounded" placeholder="Email address or username" value={email} onChange={e=>setEmail(e.target.value)} />
        <input className="w-full my-2 p-2 rounded" placeholder="Password" type="password" value={password} onChange={e=>setPassword(e.target.value)} />
        ...
        <div className="mt-2">
          <a href="#" onClick={async()=>{ const emailPrompt = prompt('Enter your email to reset'); if(emailPrompt) { await supabase.auth.resetPasswordForEmail(emailPrompt, { redirectTo: window.location.origin + '/profile' }); alert('Password reset sent'); }}}>Forgotten password?</a>
        </div>
      </div>
    </div>
  );
