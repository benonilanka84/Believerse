...
export default function Dashboard() {
  ...
  // --- 1. VERSE LOGIC (Fixed Images) ---
  function generateDailyVisualVerse() {
    const verses = [
      { text: "I am the good shepherd. The good shepherd lays down his life for the sheep.", ref: "John 10:11", bg: "https://images.unsplash.com/photo-1484557985045-6f5c98486c90?auto=format&fit=crop&w=800" },
      { text: "The Lord is my light and my salvation; whom shall I fear?", ref: "Psalm 27:1", bg: "https://images.unsplash.com/photo-1505322022379-7c3353ee6291?auto=format&fit=crop&w=800" },
      { text: "He leads me beside still waters.", ref: "Psalm 23:2", bg: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?auto=format&fit=crop&w=800" }
    ];
  ...
  async function loadPrayerWall() {
    const { data } = await supabase.from('posts').select('id, content, profiles(full_name)').eq('type', 'Prayer').order('created_at', { ascending: false }).limit(3);
  ...
  function filterEventsByDate(date, allEvents) {
    ...
    setFilteredEvents(allEvents.filter(e => e.date === dateStr));
  }
      ...
      <div className="dashboard-grid">
        <div className="left-panel">
          ...
          <div className="panel-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}><h3 style={{ margin: 0 }}>📅 Events</h3><Link href="/events" style={{ fontSize: "12px", color: "#2e8b57", fontWeight: "600" }}>View All →</Link></div>
            ...
            {filteredEvents.length > 0 ? filteredEvents.map(e => <div key={e.id} style={{fontSize:'12px', padding:'5px', background:'#e8f5e9', marginBottom:'5px'}}>{e.title}</div>) : <div style={{fontSize:'12px', color:'#999'}}>No events.</div>}
          </div>
        </div>
