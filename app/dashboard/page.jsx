...
export default function Dashboard() {
  ...
  function generateDailyVisualVerse() {
    const verses = [
      { text: "I am the good shepherd. The good shepherd lays down his life for the sheep.", ref: "John 10:11", bg: "https://images.unsplash.com/photo-1484557985045-6f5c98486c90?w=800&q=80" },
      { text: "The Lord is my light and my salvation; whom shall I fear?", ref: "Psalm 27:1", bg: "https://images.unsplash.com/photo-1505322022379-7c3353ee6291?w=800&q=80" },
      { text: "He leads me beside still waters.", ref: "Psalm 23:2", bg: "https://images.unsplash.com/photo-1470071459604-3b5ec3a7fe05?w=800&q=80" }
    ];
    // Rotate verse by date
  ...
  async function loadSuggestedBelievers(userId) {
    // Fetch 3 random users who are NOT me
  ...
  async function loadPrayerWall() {
    // Fetch latest 3 'Prayer' type posts
  ...
  async function loadRecentChats(userId) {
    // 1. Get messages where I am sender OR receiver
  ...
  // --- 5. EVENTS (Dummy for now, can be swapped for real) ---
  function loadDummyEvents() {
    const today = new Date().toISOString().split('T')[0];
    const dummy = [{ id: 1, title: "Morning Prayer", date: today }];
    setEvents(dummy);
        ...
        <div className="left-panel">
          ...
          {verseData && (
            <div className="panel-card" style={{padding:0, overflow:'hidden', position:'relative', borderRadius:'12px', border:'none'}}>
              ...
              <div style={{display:'flex', borderTop:'1px solid #eee'}}>
                ...
                <button style={{flex:1, padding:'10px', border:'none', background:'white', cursor:'pointer', color:'#0b2e4a', fontWeight:'bold'}}>📢 Spread</button>
              </div>
            </div>
          )}
          ...
          <div className="panel-card">
            ...
            {filteredEvents.length > 0 ? (
              filteredEvents.map(e => <div key={e.id} style={{fontSize:'12px', padding:'5px', background:'#e8f5e9', marginBottom:'5px'}}>{e.title}</div>)
        ...
        <div className="center-panel">
          ...
          <div className="panel-card">
            ...
            {loadingPosts ? <p style={{textAlign:'center', padding:'20px'}}>Loading...</p> : 
             posts.length === 0 ? (
               <div style={{textAlign:'center', padding:'40px', color:'#666'}}>The Walk is quiet. Be the first to share!</div>
             ) : (
               posts.map(post => (
                 <div key={post.id} style={{border:'1px solid #eee', borderRadius:'12px', padding:'15px', marginBottom:'15px', background:'#fafafa'}}>
                       ...
                       <div style={{marginLeft:'auto', position:'relative'}}>
                         ...
                         {openMenuId === post.id && (
                           <div style={{position:'absolute', right:0, top:'25px', background:'white', border:'1px solid #eee', boxShadow:'0 4px 12px rgba(0,0,0,0.1)', borderRadius:'8px', zIndex:10, width:'100px'}}>
                             <button onClick={() => startEditing(post)} style={{width:'100%', padding:'8px', textAlign:'left', border:'none', background:'white', cursor:'pointer', fontSize:'13px'}}>Edit</button>
                   ...
                   <div style={{display:'flex', justifyContent:'space-between', marginTop:'15px', borderTop:'1px solid #eee', paddingTop:'10px'}}>
                     ...
                     <button style={{background:'none', border:'none', color:'#666', cursor:'pointer'}}>📢 Spread</button>
                   </div>
                 </div>
               ))
             )
              ...
              suggestedBelievers.map(b => (
                <div key={b.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px'}}>
                  <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                    ...
                    <span style={{fontSize:'13px'}}>{b.full_name?.split(' ')[0]}</span>
                  </div>
